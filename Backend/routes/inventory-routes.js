...
router.post('/inventory/add', async (req, res, next) => {
    const { itemId, name, quantity, description } = req.body;  // Removed receivedDate and releaseDate
    try {
        let newItem = await Inventory.findOne({ itemId });
        if (!newItem) {
            // If item does not exist in inventory, create it
            newItem = new Inventory({ itemId, name, quantity, description });
        } else {
            // If item exists, update quantity
            newItem.quantity += quantity;
        }

        newItem.lastUpdated = Date.now();  // Update lastUpdated to the current date
        await newItem.save();

        res.status(201).json(newItem);  // Return the updated item
    } catch (error) {
        res.status(400).json({ message: error.message });
    }
});
